...
// global variables
unsigned long volatile prevSW_val; // stores previous switch value
unsigned long volatile toggle; // toggle is 1, quiet is 0
...
// called at 880 Hz, every 440Hz will toggle 2 times
void SysTick_Handler(void){
	unsigned long curSW_val = (GPIO_PORTA_DATA_R & 0x08) >> 3;
	
	if (curSW_val == 1) { // if button is pressed
		if (prevSW_val == 0) {
			toggle ^= 1;
		}
		prevSW_val = curSW_val;
	} else {
		prevSW_val = curSW_val;
	}
